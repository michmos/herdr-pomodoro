...
2. **Tab bar command**
   ...
   Since - as of now - herdr plugins can't edit the tab bar directly, you need to add this to your herdr configuration file as well:
   ```toml
    [ui]
    tab_bar_right = [
      { type = "command", command = "<path-to-plugin-install-directory>/scripts/pomodoro-status.sh",
        interval_seconds = 1, timeout_seconds = 1 },
    ]
    ```
    - path should usually be: `~/.config/herdr/plugins/github/herdr-pomodoro/scripts/pomodoro-status.sh`
...
- **Status script** (`scripts/pomodoro-status.sh`) — polled by herdr's `tab_bar_right` every `interval_seconds`. It `cats` the status file and herdr uses this standard output to update the status bar. This script is deliberately as light as possible, so it can be run frequently with little CPU load.
